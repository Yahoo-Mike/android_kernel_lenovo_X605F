...
struct smbchg_chip {
	...
	struct delayed_work		pogo_report_work;
...
static int get_prop_batt_capacity(struct smbchg_chip *chip)
{
	...
	chip->ext_fg_psy = power_supply_get_by_name("ex-fg");
	if (chip->ext_fg_psy) {
						...
						enum power_supply_type type)
{
	 ...
	 */
	 //modified by jiangwy1 to discard typec current config
	 /*
...
static void sw_jeita_machine(struct smbchg_chip *chip)
{
    ...
    *******************************************/
	...
	pr_err(" batt_current = %d, batt_voltage = %d, batt_temperature = %d, batt_capacity = %d\n",batt_current,batt_voltage,batt_temperature,batt_capacity);
	...
	switch (jeita_status) {
		case SWJEITA_LOW_TEMP:
			...
			chip->batt_health = POWER_SUPPLY_HEALTH_COOL;
			break;
		case SWJEITA_NORMAL_TEMP:
		...
		case SWJEITA_NORMAL_HIGH_TEMP:
			...
			chip->batt_health = POWER_SUPPLY_HEALTH_WARM;
			break;

		case SWJEITA_COLD_TEMP:
			rc = set_prop_sw_jeita_vol_cur(chip, 4100, 0); //<0C            4.10V/0A
			chip->batt_health = POWER_SUPPLY_HEALTH_COLD;
			break;
		case SWJEITA_HIGH_TEMP:
			rc = set_prop_sw_jeita_vol_cur(chip, 4100, 0); //>50C		   4.10V/0A
			chip->batt_health = POWER_SUPPLY_HEALTH_HOT;
			break;
	...
	chip->batt_psy.get_property(&chip->batt_psy, POWER_SUPPLY_PROP_HEALTH, &prop);
	rc = chip->batt_psy.set_property(&chip->batt_psy, POWER_SUPPLY_PROP_HEALTH, &prop);
	if(rc != 0){
		printk(KERN_ERR " sw_jeita_machine set POWER_SUPPLY_PROP_HEALTH failed batt_health = %d\n", chip->batt_health);
	}
	else{
		printk(" sw_jeita_machine POWER_SUPPLY_PROP_HEALTH  change to %d\n", chip->batt_health);
	}
...
#endif
static void smbchg_pogo_report_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				pogo_report_work.work);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		vote(chip->battchg_suspend_votable, BATTCHG_USER_EN_VOTER,
				!val->intval, 0);
		chip->batt_protect = !val->intval;
		pr_err("set battery_charging_enabled : %d\n", val->intval);
		break;
...
static irqreturn_t dcin_uv_handler(int irq, void *_chip)
{
	...
	smbchg_wipower_check(chip);	
	schedule_delayed_work(&chip->pogo_report_work,msecs_to_jiffies(500));
	return IRQ_HANDLED;
}
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
	...
	if (g_boost_en_flag == 1) {
		pr_err("OTG mode skip below code");
		goto out;
	}
	rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + RT_STS, 1);
	if (rc) {
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	if (chip->jeita_temp_hard_limit >= 0) {
		rc = smbchg_sec_masked_write(chip,
			...
			JEITA_TEMP_HARD_LIMIT_BIT|BIT(2)|BIT(3)|BIT(1)|BIT(0),
...
#endif
	INIT_DELAYED_WORK(&chip->pogo_report_work, smbchg_pogo_report_work);
