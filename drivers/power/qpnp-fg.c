...
static int64_t get_batt_id(unsigned int battery_id_uv, u8 bid_info)
{
	...
	pr_err("first read battery_id_ohm = %lld\n",battery_id_ohm);
	if (battery_id_ohm >= 50000 && battery_id_ohm <= 450000)
		battery_id_ohm = 100000;
	else if (battery_id_ohm < 50000 && battery_id_ohm > 0)
		battery_id_ohm = 0;
	pr_err("read battery_id_ohm = %lld\n",battery_id_ohm);
