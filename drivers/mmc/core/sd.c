...
#include "../drivers/mmc/host/sdhci-msm.h"
...
static int mmc_sd_init_uhs_card(struct mmc_card *card)
{
	...
	pr_err("### Bayhub debug: enter mmc_sd init uhs card ###");
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end
	...
	if (card->sd_bus_speed < card->degrade_count)
			card->sd_bus_speed = 0;
	else
			card->sd_bus_speed -= card->degrade_count; 
...
out:
	kfree(status);
	pr_err("### Bayhub debug: exit mmc_sd init uhs card ###");
	return err;
}
...
try_again:
	...
	/* Skip change 1.8V */
	if((host->v18_disable && host->card->degrade) || host->legacy_mode) {
...
void bht_load(struct mmc_host *mmc_host, struct mmc_card *card)
{
	...
	if ( sdhci_bht_target_host(host))//wangpengpeng@wind-mobi.com modify for sd at 20180223
	{
		...
		pr_info("%s: Load BHT stage 3 patch RC8.2.0\n", mmc_hostname(mmc_host));
		/* clear tuning flag when need to degrade mode */
		if (card->degrade) {	
			_ggc_reset_tuning_result_for_dll(host);
			pr_info("### Bayhub debug: tuning done flag is %d when degrade ###\n",msm_host->ggc.selx_tuning_done_flag);
		}
	...
	struct mmc_card *oldcard)
{
	...
	pr_err("### Bayhub debug: enter mmc_sd init card ###");
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end		
	...
	card->degrade = 0;
	pr_err("### Bayhub debug: exit mmc_sd init card ###");
	return 0;

free_card:
	pr_err("### Bayhub debug: Set degrade flag, because init failed ###");
	if (!oldcard) {
		host->card = NULL;
