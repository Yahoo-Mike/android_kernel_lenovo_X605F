														...
														u8 num)
{
...
#ifdef  FORCE_READ_STATUS_CMD7_TIMEOUT
	{
		...
		{
			DbgErr("Force first CMD7 timeout error occur\n");
			if (card_status)
				*card_status = FALSE;
			ret = FALSE;
...
exit:
	...
	if (ret == FALSE)
		DbgErr("%s: GGC Emulator Write Fail!!\n", __FUNCTION__);
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end
	...
	if ((TRUE == ret)) {
		...
		PrintMsg("dll sela after mask=%xh\n", gg_reg_arr[4].value);
		PrintMsg("dll selb after mask=%xh\n", gg_reg_arr[5].value);

		if (raw_tx_selb) {
			*raw_tx_selb = gg_reg_arr[1].value;
			(*raw_tx_selb) <<= 32;
			*raw_tx_selb += gg_reg_arr[0].value;
			PrintMsg("raw_tx_selb:%llxh\n", *raw_tx_selb);
		}
					      ...
					      int tuning_cnt)
{
	...
	PrintMsg("%s selb:%xh,tuning_cnt:%xh\n", __FUNCTION__, selb,
		 tuning_cnt);
...
retry:
	...
	if ((tuned_phase_cnt == NUM_TUNING_PHASES) &&
		...
		while (++drv_type <= MAX_DRV_TYPES_SUPPORTED_HS200) {
			pr_debug("%s: trying different drive strength (%d)\n",
				 mmc_hostname(mmc), drv_type);
			if (card->ext_csd.raw_drive_strength & (1 << drv_type)) {
				sdhci_msm_set_mmc_drv_type(host, opcode,
							   drv_type);
				if (!drv_type_changed)
					drv_type_changed = true;
...
static int ggc_get_first_0_index(u32 val)
{
	...
	PrintMsg("oops-not find 0 index\n");
	return 0;
}
...
static int _tx_selb_inject_policy(int tx_selb, int org_selb)
{
	if ((org_selb & BIT_PASS_MASK) != BIT_PASS_MASK) {
		...
		if (sel != (sel & tx_selb)) {
			...
			DbgErr("selb_failed range:%xh  ,new tx_selb:%x\n",
			       org_selb, tx_selb);
			DbgErr("\n\n!!!=============\n");
		}
		org_selb &= tx_selb;
		...
		case 2:
			...
			break;
		default:
			PrintMsg(">= 3 point case\n");
		}
		org_selb &= tx_selb;
	}
	
	PrintMsg("will check continous 0bits: 0x%x\n", org_selb);
	if (1)
	{
		...
		if (group_cnt > 1)
		{
			int max_len_group = 0;
			int max_len = 0;
			DbgErr("After inject, selb 0x%x has %d continous 0 bits\n", org_selb, group_cnt);
			...
			DbgErr("After merge incontious 0 group, selb changed to 0x%x\n", org_selb);
		}
		else if (group_cnt > 0){
			DbgErr("After merge incontious 0 group, selb = 0x%x\n", org_selb);
		}
		else {
			DbgErr("selb 0x%x has no bit is 0\n", org_selb);			
		}
	}
...
void tx_selb_inject_policy(struct sdhci_host *host, int tx_selb)
{
	...
	PrintMsg("before inject, failed ragen 0x%x, tx_selb 0x%x\n", msm_host->ggc.ggc_cmd_tx_selb_failed_range, tx_selb);
	...
	PrintMsg("after inject %xh range:%xh\n", tx_selb,
...
int get_selb_failure_point(int start, u64 raw_tx_selb, int tuning_cnt)
{
	...
	PrintMsg("%s start:%d tuning_cnt:%d\n", __FUNCTION__, start,
			 tuning_cnt);
...
exit:
	PrintMsg("%s: after adjust raw_tx_selb: 0x%llx, vct 0x%x\n",
...
int _get_best_window_phase(u32 vct, int * pmax_pass_win, int shif_left_flg)
{
    ...
    if(tuning_pass_num_max == 0)
    {
        DbgErr("###### Get max pass window fail, there is no any pass phase!!\n");
        return 0;
    }
    else
    {
...
static int _ggc_get_suitable_selb_for_next_tuning(struct sdhci_host *host)
{
	...
	if (selx_failure_point_exist(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) {
		...
		PrintMsg("manual inject for all pass case\n");
		...
		PrintMsg("manual inject for all pass case, inj_tx_selb=0x%x\n", inj_tx_selb);
		selb = get_best_window_phase(inj_tx_selb, NULL);
		PrintMsg("select selb %d for all pass case\n", selb);
	}
	return selb;
}
...
bool ggc_hw_inject_ext(struct sdhci_host * host, bool *card_status, u32 sel200, u32 sel100, bool writetobh201)
{
	...
	u8 data[512];
	t_gg_reg_strt gg_reg_arr[10];
	PrintMsg("%s sel200:%xh,sel100:%xh\n", __FUNCTION__, sel200, sel100);
	...
	else
	{
		if (1)
		{
			u32 i = 0;
			u32 reg;
			PrintMsg("%s: dump config data instead write to bh201\n", __FUNCTION__);
			for (i = 0; i < 128; i++)
			{
				memcpy(&reg, data+i*sizeof(u32), sizeof(u32));
				PrintMsg("    ggc_reg32[%03d]=0x%08x\n", i, reg);
...
{
	{
		...
		if((FALSE == ret) && (FALSE == card_status))
		{
			...
			PrintMsg("inject again when hw inject\n");
			...
			{
					DbgErr("pass windows too small,reinit recursive\n");
					return FALSE;
			}
...
bool get_next_dll_voltage(int cur, int * next, u32 * dll_voltage_unlock_cnt, int * dll_voltage_scan_map)
{
    ...
    if(0 == cur_flg)
    {
        DbgErr("no find available voltage\n");
        return FALSE;
    }
    else
    {
        *next = min_idx;
        DbgErr("next available voltage %d\n", min_idx);
        return TRUE;
    }
}
			       ...
			       u64* raw_tx_selb)
{
	...
	if(card_status && read_status)
		PrintMsg("card_status,read_status:%x %x\n", *card_status, *read_status);
	return ret;
}
	...
	if(FALSE == card_status) {
		if(TRUE == read_status) {
			...
			if (cmd7_timeout_issue_test_flag)
			{
				...
				PrintMsg("inject selb %03x for force CMD7 read timeout error\n", selb);
				cmd7_timeout_issue_test_flag = FALSE;
			}
#else
			PrintMsg("inject selb %03x for CMD7 read timeout\n", selb);			
#endif
			tx_selb_inject_policy(host, selb);
		} else {
			PrintMsg("read status failedA!!\n");
			PrintMsg("============ %s dll:%xh failed ============\n", __FUNCTION__, cur_sela);
		}
		ret = FALSE;
		goto exit;
	} else {
...
#define GGC_SW_SELB_TUNING_LOOP 3
	...
	if (ggc->tuning_cmd7_timeout_reinit_flg) {
		...
		PrintMsg("Tuning will start from sela: 0x%x, selb: 0x%x where CMD7 timeout occur\n", dll_sela_basis, cur_selb);
	}
#else
	ggc->tuning_cmd7_timeout_reinit_flg = 0;
#endif
	...
	for (dll_sela_cnt = 0; dll_sela_cnt < TUNING_PHASE_SIZE; dll_sela_cnt++) {
		...
		PrintMsg("============ %s select sela dll: %x, selb dll: %x ============\n", __FUNCTION__, cur_sela, cur_selb);
		if((NO_TUNING != psela_tuning_result[cur_sela]))
		{
			PrintMsg("sela tuning=%d already tuning,so skip it\n", cur_sela);
			continue;
		}
rescan_selb:
		host_cmddat_line_reset(host);
		...
		if (dll_sela_cnt == 0) {
...
#ifdef FORCE_FIRST_CMD19_FAIL_ERROR
			{
				...
				{
					PrintMsg("force first cmd19 fail to debug dll voltage change, cur_dll_voltage_idx=%d", ggc->cur_dll_voltage_idx);
			...
			if (ret == FALSE || FALSE == card_status) {
				DbgErr("Error when output_tuning, sd_tuning set sela,selb fail at phase %d, ret = %d, card_status=%d\n",
							 cur_sela, ret, card_status);
				if (FALSE == card_status) {
					int selb =	BIT_PASS_MASK;
					selb &= ~ GENERATE_IDX_VALUE(cur_selb);
					DbgErr("inject selb %d for update sela/selb failed\n", selb);
					tx_selb_inject_policy(host, selb);
					_ggc_update_cur_setting_for_sw_selb_tuning(host, ggc->ggc_cmd_tx_selb_failed_range); //update cur
					ggc_hw_inject_may_recursive(host, ggc->ggc_cmd_tx_selb_failed_range, ggc->ggc_cmd_tx_selb_failed_range, TRUE);
					...
					if (((11 - get_bit_number(ggc->ggc_cmd_tx_selb_failed_range)) >= 5))
					{
						...
						DbgErr("pass windows too small,reinit when set current sela & selb failed\n");
						if (current_ds < 7)
						...
						ret = FALSE;
						DbgErr("will change driver strength from %d to %d\n", 
									 current_ds,
		...
		if((11 - get_bit_number(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) >= 5) {
			u8 current_ds = (u8)(g_gg_reg_cur[15] >> 28);
			DbgErr("pass windows too small after result calculate, reinit\n");
			if (current_ds < 7)
				msm_host->ggc.driver_strength_reinit_flg = current_ds + 1;
			else
				msm_host->ggc.driver_strength_reinit_flg = 7;
		...
		if(FALSE == ret) {
			DbgErr("cmd7 timeout fail,reinit\n");
			...
			if((11 - get_bit_number(msm_host->ggc.ggc_cmd_tx_selb_failed_range)) >= 5) {
				u8 current_ds = (u8)(g_gg_reg_cur[15] >> 28);
				DbgErr("pass windows too small after cmd7 timeout inject\n");
				DbgErr("do driver strength reinit instead cmd7 timeout reinit\n");
		...
		if ((rescan_selb_cnt < GGC_SW_SELB_TUNING_LOOP) &&
			...
			PrintMsg("rescan cnt %d, ggc_cmd_tx_selb_failed_range=0x%x\n", 
							 rescan_selb_cnt, 
							 msm_host->ggc.ggc_cmd_tx_selb_failed_range);
			goto rescan_selb;
		}
		...
		// calculate selb
		{
			...
			}
			{
				...
				if (host->info.sw_cur_setting.sd_access_mode ==
						SD_FNC_AM_SDR50
						|| (host->info.
								sw_cur_setting.sd_access_mode ==
								SD_FNC_AM_SDR104
								&&
								(get_pass_window_weight
								 (host->host->cfg->
									output_pfm.sw_sela_sdr104.
									fail_phase) != 11))) {
					// move for host
...
void ggc_tuning_result_reset(struct sdhci_host *host)
{
	...
	PrintMsg("%s: will clear all tuning results\n", __FUNCTION__);
	_ggc_reset_tuning_result_for_dll(host);
	...
	msm_host->sdr50_notuning_sela_rx_inject = 0x71f;
}
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 begin
static int sdhci_bht_execute_tuning(struct sdhci_host *host, u32 opcode)
{
	if (sdhci_bht_target_host(host)) {	//wangpengpeng@wind-mobi.com modify for sd at 20180223
		...
		if (msm_host->ggc.selx_tuning_done_flag) {
			PrintMsg("GGC tuning is done, just do msm host tuning");
			if (is_bus_mode_sdr104(host)) {
				ret = sdhci_bht_sdr104_execute_tuning(host, 0x13); 
			} else {
				ret = sdhci_bht_sdr50_execute_tuning(host, 0x13); 
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 begin
		if (sdhci_bht_target_host(host)) {
			...
			pr_info("%s: pin 141 status is %d\n", mmc_hostname(host->mmc), gpio_get_value(141));			
		}
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end
	...
	if (irq_status & CORE_PWRCTL_BUS_OFF) {
			...
			//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 begin
		{
			if (sdhci_bht_target_host(host)) {
				...
				gpio_direction_output(141, 0);	//set PWR GPIO off
				pr_info("%s: pin 141 status is %d\n", mmc_hostname(host->mmc), gpio_get_value(141));
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	if (sdhci_bht_target_host(host)) {
		pr_info("%s: apply bht probe patch\n", mmc_hostname(host->mmc));				
		_ggc_reset_tuning_result_for_dll(host);
		ggc_dll_voltage_init(host);
		ggc_tuning_result_reset(host);
		ggc_chip_init(host);
	} else {
		os_memset(&msm_host->ggc, 0, sizeof(ggc_platform_t));
	}
