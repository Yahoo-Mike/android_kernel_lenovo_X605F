...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 begin
static bool _gg_emulator_read_only(
   struct sdhci_host *host,
    u8 * in_data,
    u32 datalen
)
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end
bool get_gg_reg_cur(struct sdhci_host *host, u8 * data, t_gg_reg_strt * gg_reg_arr, u8 num)
{
    u8 get_idx = 0;
    bool ret = FALSE;
    ...
    {
        (gg_reg_arr + get_idx)->value = read_ram_bits_ofs_mask(data, (gg_reg_arr + get_idx));
    }
exit:
    return ret;
}
...
bool chg_gg_reg(struct sdhci_host *host,  u8 * data, t_gg_reg_strt * gg_reg_arr, u8 num)
{
    ...
    for(chg_idx = 0; chg_idx < num; chg_idx++)
    {
        ...
        cfg_write_bits_ofs_mask(data, (gg_reg_arr + chg_idx), (gg_reg_arr + chg_idx)->value);
    }
...
void chg_gg_reg_cur_val(u8 * data, t_gg_reg_strt * gg_reg_arr, u8 num, bool b_sav_chg)
{
    u8 chg_idx = 0;
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end
    DbgInfo(MODULE_SD_HOST, FEATURE_SDREG_TRACEW, NOT_TO_RAM, "%s sela:%xh,selb:%xh\n", __FUNCTION__, sela, selb);
    ...
    os_memcpy(&gg_reg_arr[6], &internl_tuning_en_100m, sizeof(t_gg_reg_strt));
    os_memcpy(&gg_reg_arr[7], &internl_tuning_en_200m, sizeof(t_gg_reg_strt));
...
//BH201LN driver--sunsiyuan@wind-mobi.com modify at 20180326 end
    ...
    if((TRUE == ret))
    {
        ...
        if(tx_selb)
        {
            gen_array_data(gg_reg_arr[0]. value, gg_reg_arr[1]. value, tx_selb);
            PrintMsg("tx_selb:%xh\n", *tx_selb);
        }
        if(all_selb)
        {
            gen_array_data(gg_reg_arr[2]. value, gg_reg_arr[3]. value, all_selb);
...
u32 tx_selb_failed_history_get(struct sdhci_host *host)
{
    ...
    else
    {
        ...
        DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM, "%s %xh\n", __FUNCTION__, g_tx_selb_failed_history_sdr50);
...
void tx_selb_failed_tb_update(struct sdhci_host *host, int sela, u32 val)
{
    if(is_bus_mode_sdr104(host))
    {
        ...
        DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM, "%s %xh %xh\n", __FUNCTION__, val, g_tx_selb_tb_sdr104[sela]);
    }
    else
    {
...
void all_selb_failed_history_update(struct sdhci_host *host, int sela, u32 val)
{
    if(is_bus_mode_sdr104(host))
    {
        ...
        DbgInfo(MODULE_SD_HOST, FEATURE_VENREG_TRACEW, NOT_TO_RAM, "%s %xh %xh\n", __FUNCTION__, val, g_all_selb_tb_sdr104[sela]);
    }
    else
    {
...
// type[0] = 0: if 011110, right 1 index is the middle value, type[1] = 1: first_i valid
...
// type[0] = 0: if 011110, right 1 index is the middle value, type[1] = 1: first_i valid
void chk_arr_max_win(u8 * tuning_win, u8 first_i, u8 * mid_val, u8 * max_pass_win, chk_type_t type)    
{
   ...
   
    {
	 ret = sd_tuning_sw(host);  
   
        {
            if(status_ret)
            {
                *status_ret = gg_tuning_result(host, tx_selb, all_selb);
            }
            else
            {
...
void get_defualt_setting(u8 * data)
{
    ...
    g_def_delaycode_100m = cfg_read_bits_ofs_mask(data, &dll_delay_100m_backup);
    g_def_delaycode_200m = cfg_read_bits_ofs_mask(data, &dll_delay_200m_backup);
...
bool  get_refine_sel(struct sdhci_host *host , u32 tga, u32 tgb, u32 * rfa, u32 * rfb)
{
    ...
    for(i = 0; i < TUNING_PHASE_SIZE; i++)
    {
        ...
        DbgInfo(MODULE_SD_HOST, FEATURE_CARD_INIT, NOT_TO_RAM, "%x len:%xh\n", selb, len_tb[selb]);
        selb++;
...
next:
        if((len_tb[lft1] + len_tb[lft2]) >= (len_tb[rt1] + len_tb[rt2]))
        {
            *rfb = lft2;
        ...
        {
            PrintMsg("refine point is failed point, so no change\n");
            *rfa = tga;
            *rfb = tgb;
        }
    }
...
//init
    ...
    {
        host_cmddat_line_reset(host);
        cur_dll_sela = (dll_sela_cnt + dll_sela_basis) % TUNING_PHASE_SIZE;
        PrintMsg("=== ot select sela: %x, selb: %x ===\n", cur_dll_sela, selb_cur);
	if((FALSE ==is_bus_mode_sdr104(host)) && 1==msm_host->sdr50_notuning_sela_inject_flag && GET_IDX_VALUE(msm_host->sdr50_notuning_sela_rx_inject,cur_dll_sela))
	{
		PrintMsg("skip %d\n",cur_dll_sela);
		tuning_error_type[cur_dll_sela] = READ_STATUS_FAIL_TYPE;
           	goto cur_sela_failed;
	}
        ret = ggc_set_output_tuning_phase(host, cur_dll_sela, update_selb(host, selb_cur));


        if(ret == FALSE)
        {
            DbgErr("Error when output_tuning,  sd_tuning set sela,selb fail at phase %d\n", cur_dll_sela);
        ...
        if(TRUE == status_ret)
        {
            ...
            all_selb_failed_history_update(host, cur_dll_sela, all_selb);
            if(get_bit_number(tx_selb) >= 6)  	//skip bad tx_selb
            {
                tx_selb_failed_tb_update(host, cur_dll_sela, tx_selb);
                tx_selb_failed_history_update(host, tx_selb);
                tx_selb = tx_selb_failed_history_get(host);
                phase_str(tuning_win, tx_selb);
                PrintMsg("### tx selb[%xh]: %s \n", tx_selb, tuning_win);
                bits_generate_array(tuning_win, tx_selb);
                no_fail_p(tuning_win, &mid_val, &selb_tuning_pass_num);
                selb_cur = mid_val;
            }
            else
            {
...
next_dll_sela:
	if((FALSE ==is_bus_mode_sdr104(host)) && msm_host->sdr50_notuning_crc_error_flag)
		{
			u32 fp=0;
			fp= GENERATE_IDX_VALUE(cur_dll_sela);
		        fp |=  GENERATE_IDX_VALUE((cur_dll_sela+ 1) % TUNING_PHASE_SIZE);
        		fp |=  GENERATE_IDX_VALUE((cur_dll_sela+ 10) % TUNING_PHASE_SIZE);
...

    {
        ...
        for(i = 0; i < TUNING_PHASE_SIZE; i++)
        {
            ...
            phase_str(all_str, all_selb_failed_history_get(host, i));
            phase_str(tx_str, tx_selb_failed_tb_get(host, i));
            PrintMsg("DLL sela[%x]  all selb: %s   tx selb: %s [%xh,%xh] %s\n", i,
                     all_str, tx_str,
        ...
        }
        {
            ...
            u32 cfg = 0;
            PrintMsg("inject sw selb & merge tx_selb failed point to all_selb\n");
            for(i = 0; i < TUNING_PHASE_SIZE; i++)
            {
        ...
        // calculate selb
        {
            ...
            PrintMsg("---selb merge---\n");
            {
                if(is_bus_mode_sdr104(host))
                {
                    u32 cfg =0x7ff;
                    tx_selb &= cfg;
                    PrintMsg("tx selb:%xh SDR104 inject:%xh merge tx_selb:%xh\n", tx_tmp, cfg, tx_selb);
                }
               else
                {
                    u32 cfg = 0x7ff;
...

            {
                if(0 == tx_selb)
                {
                    DbgErr("no valid  selb window, all failed, so force fixed phase sela selb to default\n");
                    target_selb = get_config_selb_setting(host) ;
                    target_sela = get_config_sela_setting(host);
                    goto final;
                }
...
final:
	PrintMsg("######### hw select mode - sela dll: %x, selb dll: %x \n",  target_sela, target_selb);
	ggc_set_output_tuning_phase(host, target_sela, target_selb);
...
static int sdhci_bht_execute_tuning(struct sdhci_host *host, u32 opcode){
	if ( sdhci_bht_target_host(host) )
	{ //wangpengpeng@wind-mobi.com modify for sd at 20180223
		u8 tw=0;
		int ret=0;
		struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
		struct sdhci_msm_host *msm_host = pltfm_host->priv;
...
/**************************************************************************/
/***  end BH201LN driver for TB-X605F 					***/
/**************************************************************************/
