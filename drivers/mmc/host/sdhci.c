...
static void sdhci_card_event(struct mmc_host *mmc)
{
	...
	pr_err("%s: sdhci_do_get_cd return %d!\n",
		mmc_hostname(host->mmc), present);
...
static irqreturn_t sdhci_thread_irq(int irq, void *dev_id)
{
	...
	if (isr & (SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE)) {
		if (isr & SDHCI_INT_CARD_INSERT)
			pr_err("%s: card insert int occur.\n",
					 mmc_hostname(host->mmc));
		if (isr & SDHCI_INT_CARD_REMOVE)
			pr_err("%s: card remove int occur.\n",
					 mmc_hostname(host->mmc));
