...
void typec_enable(void)
{
	 if(gpio_is_valid(typec_en_pin)){
		gpio_request(typec_en_pin,"typec_en");
		gpio_direction_output(typec_en_pin,0);
		pr_err("typec_enabled ok\n");
		gpio_free(typec_en_pin);
	}
	 else{
		pr_err("typec_en_pin is invalid\n");
	 }
	 return;
}
void typec_disable(void)
{
	 if(gpio_is_valid(typec_en_pin)){
		gpio_request(typec_en_pin,"typec_en");
		gpio_direction_output(typec_en_pin,1);
		pr_err("typec_en_disabled ok\n");
		gpio_free(typec_en_pin);
	}
	 else{
		pr_err("typec_en_pin is invalid\n");
...
void boost_enable(void)
{
	 if(gpio_is_valid(boost_en_pin)){
		gpio_request(boost_en_pin,"boost_en");
		gpio_direction_output(boost_en_pin,1);
		pr_err("boost_enabled ok\n");
		gpio_free(boost_en_pin);
	}
	 else{
		pr_err("boost_en_pin is invalid\n");
	 }
	 return;
}
...
void otg_flag_enable(void)
{
	 if(gpio_is_valid(otg_flag_pin)){
		gpio_request(otg_flag_pin,"otg_flag");
		gpio_direction_output(otg_flag_pin,1);
		pr_err("otg_flag_enabled ok\n");
		gpio_free(otg_flag_pin);
	}
	 else{
		pr_err("otg_flag_pin is invalid\n");
	 }
	 return;
}
void otg_flag_disable(void)
{
	 if(gpio_is_valid(otg_flag_pin)){
		gpio_request(otg_flag_pin,"otg_flag");
		gpio_direction_output(otg_flag_pin,0);
		pr_err("otg_flag_disabled ok\n");
		gpio_free(otg_flag_pin);
	}
	 else{
		pr_err("otg_flag_pin is invalid\n");
...
void pogo_enable(void)
{
	 if(gpio_is_valid(pogo_en_pin)){
		gpio_request(pogo_en_pin,"pogo");
		gpio_direction_output(pogo_en_pin,1);
		pr_err("pogo_enabled ok\n");
		gpio_free(pogo_en_pin);
	}
	 else{
		pr_err("pogo_pin is invalid\n");
	 }
	 return;
}
void pogo_disable(void)
{
	 if(gpio_is_valid(pogo_en_pin)){
		gpio_request(pogo_en_pin,"pogo");
		gpio_direction_output(pogo_en_pin,0);
		pr_err("pogo_disabled ok\n");
		gpio_free(pogo_en_pin);
	}
	 else{
		pr_err("pogo_pin is invalid\n");
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	typec_en_pin = of_get_named_gpio(node,"msmdwc,typec_en_pin", 0);
	typec_enable();
	boost_en_pin = of_get_named_gpio(node,"msmdwc,boost_en_pin", 0);
	boost_disable();
	otg_flag_pin = of_get_named_gpio(node,"msmdwc,otg_flag_pin", 0);
	otg_flag_disable();
	pogo_en_pin = of_get_named_gpio(node,"msmdwc,pogo_en_pin", 0);
	pogo_disable();
