 ...
 *  fengnan@wind-mobi.com add fw update at 20180518
 * Driver for the T316 
...
//#define DEBUG
#define DRIVER_NAME "abov,LTE-T316"
#define LTE_SAR_FW_NAME "LTE_A96T316_fw.bin"   //fengnan@wind-mobi.com change fw update file name at 20180511
...
static struct i2c_client *g_adapter0_I2Cclient=NULL;
...
static int wifi_status = 198;
static int tel_status = 199;
static pT316XX_lte_t abov_sar_ptr;

typedef struct T316
{
  pbuttonInformation_t pbuttonInformation;
	pT316_platform_data_t hw; /* specific platform data settings */
} T316_t, *pT316_t;

#if 0
static void ForcetoTouched(pT316XX_lte_t this)
{
  pT316_t pDevice = NULL;
...
static void T316XX_schedule_work(pT316XX_lte_t this, unsigned long delay);

static int write_register(pT316XX_lte_t this, u8 address, u8 value)
{
...
static int read_register(pT316XX_lte_t this, u8 address, u8 *value)
{
  struct i2c_client *i2c = 0;
  s32 returnValue = 0;
...
{
	int update_loop;
	pT316XX_lte_t this = abov_sar_ptr;
	...
	printk("[T316]abov_fw_update\n");
	printk("[T316]fw_name= %s\n",fw_name);
	rc = request_firmware(&fw, fw_name, this->pdev);
	if (rc < 0) {
		printk("[T316]Request firmware failed - %s (%d)\n",
				this->fw_name, rc);
		return rc;
	}
	printk("[T316]force= %d\n",force);
	...
	printk("[T316]fw_version= %d,fw_modelno=%d\n",fw_version,fw_modelno);
...
{
	pT316XX_lte_t this = container_of(work, T316XX_t, fw_update_work);
...
static long i2c0_adapter_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case SARSENSOR_SET_MODE:
			...
			if(enable){
				err = i2c_smbus_write_byte_data(g_adapter0_I2Cclient,T316_CTRL_MODE,1);
				if(err < 0)	
					return err;
			}
			else{
				err = i2c_smbus_write_byte_data(g_adapter0_I2Cclient,T316_CTRL_MODE,2);
				if(err < 0)
					return err;
			}
		...
		case SARSENSOR_GET_DIFF_CH0:
			data_lsb = i2c_smbus_read_byte_data(g_adapter0_I2Cclient,T316_DIFF_LSB_CH0);
			data_msb = i2c_smbus_read_byte_data(g_adapter0_I2Cclient,T316_DIFF_MSB_CH0);
		...
		case SARSENSOR_GET_DIFF_CH1:
			data_lsb = i2c_smbus_read_byte_data(g_adapter0_I2Cclient,T316_DIFF_LSB_CH1);
			data_msb = i2c_smbus_read_byte_data(g_adapter0_I2Cclient,T316_DIFF_MSB_CH1);
			data_temp = (data_msb << 8) | data_lsb;
			if(copy_to_user(ptr, &data_temp,sizeof(data_temp))){
				err = -EFAULT;
				return err;
		...
		case SARSENSOR_GET_INT_STATUS:
			int_status = i2c_smbus_read_byte_data(g_adapter0_I2Cclient,T316_IRQSTAT_REG);
		...
		case SARSENSOR_GET_VENDOR_ID:
			ic_vendor = i2c_smbus_read_byte_data(g_adapter0_I2Cclient,T316_VENDOR_ID);
...
/*! \fn static int read_regStat(pT316XX_lte_t this)
 ...
 */
 static int read_irq_status(pT316XX_lte_t this)
{
	u8 data = 0;
	if (this) {
		if (read_register(this,T316_IRQSTAT_REG,&data) == 0)
		return (data & 0x00FF);
	}
	return 0;
}
static int read_channel(pT316XX_lte_t this)
{
	u8 data = 0;
	int ret = 0;
	
	if (this) {
		if (read_register(this,T316_CH_REG,&data) == 0)
		{
			ret = ((int)data & 0x00FF);
			return ret;
		}else{
			//dev_info(this->pDevice, "read_channel() err data=%d\n",data); 	
			pr_info("[T316]: %s - read_channel() err data=%d \n",__func__, data); 
		}
	}
	return 0;
}

/*! \fn static int initialize(pT316XX_lte_t this)
 * \brief Performs all initialization needed to configure the device
 * \param this Pointer to main parent struct 
 * \return Last used command's return value (negative if error)
 */
static int initialize(pT316XX_lte_t this)
{
  ...
  if (this) {
   ...
   write_register(this,T316_CTRL_MODE,0);
   write_register(this,T316_CH_REG,3);
    write_register(this,T316_GAIN,2);
 
    write_register(this,T316_CALIBRATION,1);
 ...
 * \param this Pointer to main parent struct 
 */
static void LTE1_touchProcess(pT316XX_lte_t this)
{
  int counter = 0;
  ...
  if (this && (pDevice = this->pDevice))
  {
    ...
    if (unlikely( (buttons==NULL) || (input==NULL) )) {
	  ...
	  printk("[T316XX]LTE1_touchProcess status = %d\n",status);
	  near_far= (status & 0x1);
	  printk("[T316XX]LTE1_touchProcess near_far = %d\n",near_far);
      switch (pCurrentButton->state) {
        case IDLE: /* Button is not being touched! */
            if(near_far){
				printk("[t316]cap button 1 touched\n");
                input_report_key(input, pCurrentButton->keycode, 1);
                input_sync(input);
                pCurrentButton->state = ACTIVE;
                // msleep(1);
                ...
                printk("Button 1 already released.\n");
            }
          	break;
        case ACTIVE: /* Button is being touched! */ 
            if(!near_far){
				printk("[t316]cap button 1 released\n");
                input_report_key(input, pCurrentButton->keycode, 0);
                input_sync(input);
                ...
                printk("Button 0 still touched.\n");
            }
          break;
		  
        default: /* Shouldn't be here, device only allowed ACTIVE or IDLE */
			printk("[T316]LTE1_touchProcess pCurrentButton->state = %d\n",pCurrentButton->state);
          break;
      };
    }
  	
}
static void LTE2_touchProcess(pT316XX_lte_t this)
{
  int counter = 0;
  int numberOfButtons = 0;
  pT316_t pDevice = NULL;
  ...
  if (this && (pDevice = this->pDevice))
  {
	...
	if (unlikely( (buttons==NULL) || (input==NULL) )) {
	  ...
	  printk("[T316XX]LTE2_touchProcess status = %d\n",status);
	  near_far= (status & 0x2);
	  printk("[T316XX]LTE2_touchProcess near_far = %d\n",near_far);
	  switch (pCurrentButton->state) {
		case IDLE: /* Button is not being touched! */
			if(near_far){
				printk("[t316]cap button 2 touched\n");
				input_report_key(input, pCurrentButton->keycode, 1);
				input_sync(input);
				pCurrentButton->state = ACTIVE;
				// msleep(1);
				...
				printk("Button 2 already released.\n");
			}
			break;
		case ACTIVE: /* Button is being touched! */ 
			if(!near_far){
				printk("[t316]cap button 2 released\n");
				input_report_key(input, pCurrentButton->keycode, 0);
				input_sync(input);
				pCurrentButton->state = IDLE;
				//msleep(1);
				...
				printk("Button 2 still touched.\n");
			}
		  break;
		  
		default: /* Shouldn't be here, device only allowed ACTIVE or IDLE */
			printk("[T316]LTE2_touchProcess pCurrentButton->state = %d\n",pCurrentButton->state);
		  break;
	  };
	}
...
static unsigned int T316_irq_gpio = 0;
static unsigned int irq_num;
static int of_get_T316_platform_data(struct device *dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "abov,LTE-T316");
	//dev_info(dev,"T316 -- of_get_T316_platform_data \n");
	pr_info("[T316]: %s - of_get_T316_platform_data \n",__func__);
	if (node) {
	irq_num = of_get_named_gpio_flags(node,"above,irq-gpio", 0, &flags);
		//dev_info(dev,"T316 -- T316XX_init: irq_num=%d, \n",irq_num);
		pr_info("[T316]: %s -  irq_num=%d \n",__func__,irq_num);
		if (gpio_is_valid(irq_num)) {
       		 gpio_free(irq_num);
       		 ret = gpio_request(irq_num, "lte-sar-sensor");

        	if (ret) {
            			printk("[t316]Could not request pwr gpio.\n");
            			return ret;
        		}
    		}

		gpio_direction_input(irq_num);
		//T316_irq_gpio = irq_num;
		T316_irq_gpio = irq_of_parse_and_map(node, 0);
		//T316_irq_gpio = of_get_named_gpio_flags(node,
		//									"above,irq-gpio", 0, &flags);
		//dev_info(dev,"T316_irq_gpio=%d\n",T316_irq_gpio);
		pr_info("[T316]: %s -  T316_irq_gpio=%d \n",__func__,T316_irq_gpio);
		if (T316_irq_gpio < 0) {
			printk("T316 get interrupts fail!\n");	
			return -1;
		} 
...
static int T316_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
...
static char t316_reg[33] = {0x00,0x01,0x02,0x03,0x06,0x07,0x08,0x09,0x0c,0x0d,0x0e,0x0f,
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	u8 val=0;
	int regist = 0;
	pT316XX_lte_t this = dev_get_drvdata(dev);
	int reg_num = 0;

	//dev_info(this->pdev, "Reading register\n");
	pr_info("[T316]: %s -  Reading register \n",__func__);
	...
	for(reg_num=0; reg_num<33; reg_num++){
		read_register(this, t316_reg[reg_num], &val);
		pr_info("[t316]: %s - Register(0x%2x) data(0x%2x)\n",__func__, t316_reg[reg_num], val);
	}
...
//fengnan@wind-mobi.com add at 20180517 begin
...
//fengnan@wind-mobi.com add at 20180517 end
 ...
 */
static int T316_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	
	pT316XX_lte_t this = 0;
	pT316_t pDevice = 0;
	pT316_platform_data_t pplatData = 0;
	struct input_dev *input = NULL;
    int i;
//+bug 271433 ,zwk.wt ,20170620,add sar sensor read i2c
	int err = 0;
	//unsigned char value = 0;  //fengnan@wind-mobi.com delete at20180612
	bool isForceUpdate;
//fengnan@wind-mobi.com delete at20180612 beign
	...
	//}
	//printk("0x03 = 0x%x\n\n",value);
//fengnan@wind-mobi.com delete at20180612  end

	err = abov_detect(client);
	if (err == 0) 
	{
		printk("abov_detect return  err = %d\n\n",err);  //fengnan@wind-mobi.com delete at20180612
		return -ENODEV;
	}
	if(err == 2)
	{
	...
	// client->addr=0x20;
	of_get_T316_platform_data(&client->dev);
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_READ_WORD_DATA))
		return -EIO;
	this = kzalloc(sizeof(T316XX_t), GFP_KERNEL); /* create memory for main struct */
    abov_sar_ptr = this;

	if (this)
	{
		//dev_info(&client->dev, "Begin initialized Device \n");
		pr_info("[T316]: %s -  Begin initialized Device \n",__func__);
		...
		* (e.q. if suspend reset device) */
		...
		//dev_info(this->pdev, "T316XX_init: this->irq=%d, T316_irq_gpio=%d,gpio-irq=%d\n",this->irq,T316_irq_gpio,gpio_to_irq(irq_num));
		pr_info("[T316]: %s -  this->irq=%d, T316_irq_gpio=%d,gpio-irq=%d \n",__func__,this->irq,T316_irq_gpio,gpio_to_irq(irq_num));
		/* do we need to create an irq timer after interrupt ? */
		this->useIrqTimer = 0;
		/* Setup function to call on corresponding reg irq source bit */
		if (MAX_NUM_STATUS_BITS>= 2)
		{
	        this->statusFunc[0] = LTE1_touchProcess; /* LTE1 */
			this->statusFunc[1] = LTE2_touchProcess; /* LTE2  */
		}
		printk("T316_probe 0\n");
		...
		this->pDevice = pDevice = kzalloc(sizeof(T316_t), GFP_KERNEL);
        ...
        this->fw_name = LTE_SAR_FW_NAME;     //fengnan@wind-mobi.com change fw update file name at 20180511
...
/*! \fn static int T316_remove(struct i2c_client *client)
 * \brief Called when device is to be removed
 * \param client Pointer to i2c_client struct
 * \return Value from T316XX_remove()
 */
static int T316_remove(struct i2c_client *client)
{
	
	pT316_t pDevice = 0;
	pT316XX_lte_t this = i2c_get_clientdata(client);
	if (this && (pDevice = this->pDevice))
	{
	...
	return T316XX_remove(this);
}

#if 1
/*====================================================*/
/***** Kernel Suspend *****/
static int T316_suspend(struct i2c_client *client,pm_message_t mesg)
{
  pT316XX_lte_t this = i2c_get_clientdata(client);
  write_register(this,T316_CTRL_MODE,1);
  return 0;
}
/***** Kernel Resume *****/
static int T316_resume(struct i2c_client *client)
{
  pT316XX_lte_t this = i2c_get_clientdata(client);
  write_register(this,T316_CTRL_MODE,0);
  return 0;
...
#if 1
/*====================================================*/
static struct i2c_device_id T316_idtable[] = {
	{ DRIVER_NAME, 0 },
	{ }
};
...
static const struct of_device_id T316_dt_ids[] = {
	{ .compatible = "abov,LTE-T316", },
	{ }
};
...
static struct i2c_driver T316_driver = {
	.probe = T316_probe,
	.remove = T316_remove,
	.detect = T316_i2c_detect,
	.driver.name = DRIVER_NAME,
	.driver = {
		   .name = DRIVER_NAME,
		   .of_match_table = T316_dt_ids,
		   },
	.id_table = T316_idtable,
	//.address_list = (const unsigned short *)forces,
#if 1// defined(USE_KERNEL_SUSPEND)
  .suspend  = T316_suspend,
  .resume   = T316_resume,
#endif
};
...
{
  unsigned long flags;
  printk("[T316XX]T316XX_schedule_work 1\n");
  if (this) {
	 APS_ERR("T316XX_schedule_work()\n");
   ...
   printk("[T316XX]T316XX_schedule_work 6\n");
  }
  else{
	
	printk("[T316XX]T316XX_schedule_work 7\n");
    	APS_ERR("T316XX_schedule_work, NULL pT316XX_lte_t\n");
	
      }
} 

static irqreturn_t T316XX_irq(int irq, void *pvoid)
{
	
	pT316XX_lte_t this ;
	printk("[T316XX]T316XX_irq \n");

	if (pvoid) {

	this = (pT316XX_lte_t)pvoid;
   	T316XX_schedule_work(this,0);
		
   		
	}
	else
		APS_ERR("T316XX_irq, NULL pvoid\n");
	
	return IRQ_HANDLED;
}

static void T316XX_worker_func(struct work_struct *work)
{
  pT316XX_lte_t this = 0;
  int status =0;
  int counter = 0;
  printk("[T316XX]T316XX_worker_func \n");
  if (work) {
    this = container_of(work,T316XX_t,dworker.work);
    if (!this) {
      APS_ERR("T316XX_worker_func, NULL T316XX_t\n");
      return;
    }
	status = read_channel(this);
   printk("[T316XX]T316XX_worker_func status=%d\n",status);
...
//	printk("[T316XX]T316XX_worker_func 9\n");
   if (unlikely(this->useIrqTimer ))
    { /* Early models and if RATE=0 for newer models require a penup timer */
      /* Queue up the function again for checking on penup */
		T316XX_schedule_work(this,msecs_to_jiffies(this->irqTimeout));
    }
  } else {
      APS_ERR("T316XX_worker_func, NULL work_struct\n");
  }
  
  printk("[T316XX]T316XX_worker_func 12\n");
}
...
int T316XX_init(pT316XX_lte_t this)
{
  ...
  if (this && this->pDevice)
  {
	...
	INIT_DELAYED_WORK(&this->dworker, T316XX_worker_func);
    /* initailize interrupt reporting */
    this->irq_disabled = 0;
	//dev_info(this->pdev, "T316XX_init: %s,this->irq=%d, \n",this->pdev->driver->name,this->irq);
	
    err = request_irq(this->irq, T316XX_irq, IRQF_TRIGGER_FALLING,"lte abov_irq", this);
	//dev_info(this->pdev, "T316XX_init: err=%d, \n",err);
	pr_info("[T316]: %s -  T316XX_init: err=%d \n",__func__,err);
	  if (err) {
		  printk("irq %d busy?\n", this->irq);
		  return err;
		  }
...
static int __init T316_init(void)
{
       printk("T316_init begin !!! \n");
       return i2c_add_driver(&T316_driver);
}
static void __exit T316_exit(void)
{
	
	i2c_del_driver(&T316_driver);
}

module_init(T316_init);
module_exit(T316_exit);

MODULE_AUTHOR("Semtech Corp. (http://www.semtech.com/)");
MODULE_DESCRIPTION("mediatek,T316");
MODULE_LICENSE("GPL");
MODULE_VERSION("0.1");
