...
static int read_channel(psxT316XX_t this)
{
	...
	if (this) {
		if (read_register(this,SXT316_CH_REG,&data) == 0)
		{
			...
			//dev_info(this->pDevice, "read_channel() err data=%d\n",data); 	//fengnan@wind-mobi.com delete at 20181112
			pr_info("[sxT316]: %s - read_channel() err data=%d \n",__func__, data);     //fengnan@wind-mobi.com add at 20181112
		}
	}
	return 0;
}
...
static void WIFI_touchProcess(psxT316XX_t this)
{
  ...
  int status = 0;
  int near_far;//0--far,1--near 
  //dev_info(this->pDevice, "WIFI_touchProcess() begin\n"); 	//fengnan@wind-mobi.com delete at 20181112
  pr_info("[sxT316]: %s - WIFI_touchProcess() begin \n",__func__);            //fengnan@wind-mobi.com add at 20181112
  if (this && (pDevice = this->pDevice))
  {
...
static int of_get_sxT316_platform_data(struct device *dev)
{
	...
	pr_info("[sxT316]: %s - of_get_sxT316_platform_data \n",__func__); 	//fengnan@wind-mobi.com add at 20181112
	if (node) {
	irq_num = of_get_named_gpio_flags(node,"above,irq-gpio", 0, &flags);
		//dev_info(dev,"sxT316 -- sxT316XX_init: irq_num=%d, \n",irq_num); 	//fengnan@wind-mobi.com delete at 20181112
		pr_info("[sxT316]: %s -  irq_num=%d \n",__func__,irq_num); 	//fengnan@wind-mobi.com add at 20181112
		...
		//dev_info(dev,"sxT316_irq_gpio=%d\n",sxT316_irq_gpio); 	//fengnan@wind-mobi.com delete at 20181112
		pr_info("[sxT316]: %s -  sxT316_irq_gpio=%d \n",__func__,sxT316_irq_gpio); 	//fengnan@wind-mobi.com add at 20181112
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("[sxT316]: %s -  Reading register \n",__func__); 	//fengnan@wind-mobi.com add at 20181112
...
//+bug 271433 ,zwk.wt ,20170620,add sar sensor read i2c
	...
	if (this)
	{
		...
		pr_info("[sxT316]: %s -  Begin initialized Device \n",__func__); 	//fengnan@wind-mobi.com add at 20181112
		...
		* (e.q. if suspend reset device) */
		...
		//dev_info(this->pdev, "sxT316XX_init: this->irq=%d, sxT316_irq_gpio=%d,gpio-irq=%d\n",this->irq,sxT316_irq_gpio,gpio_to_irq(irq_num)); 	//fengnan@wind-mobi.com delete at 20181112
		pr_info("[sxT316]: %s -  this->irq=%d, sxT316_irq_gpio=%d,gpio-irq=%d \n",__func__,this->irq,sxT316_irq_gpio,gpio_to_irq(irq_num)); 	//fengnan@wind-mobi.com add at 20181112
...
int sxT316XX_init(psxT316XX_t this)
{
  ...
  if (this && this->pDevice)
  {
	...
	//dev_info(this->pdev, "sxT316XX_init: err=%d, \n",err); 	//fengnan@wind-mobi.com delete at 20181112
	pr_info("[sxT316]: %s -  sxT316XX_init: err=%d \n",__func__,err); 	//fengnan@wind-mobi.com add at 20181112
	  if (err) {
		  printk("irq %d busy?\n", this->irq);
		  return err;
		  }
