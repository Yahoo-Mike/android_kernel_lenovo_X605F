...
#if 1	/*zhangchaofan@wind-mobi.com add for fts_reset_proc enable 2018-04-25  begin*/
...
//sunjingtao@wind-mobi.com add at 20180801 begin
     if(headset_flag == 1)
    {
    	...
    	ret = fts_i2c_write_reg(data->client, 0x90, 0x01);
    	if (ret < 0)
        	FTS_ERROR("write R90 fail, ret=%d", ret);
    }
    else
    {

    	ret = fts_i2c_write_reg(data->client, 0x90, 0x00);
    	if (ret < 0)
        	FTS_ERROR("write R90 fail, ret=%d", ret);
    }
    if(charge_flag == 1)
    {
    	ret = fts_i2c_write_reg(data->client, FTS_REG_CHARGER_MODE_EN, 0x01);
    	if (ret < 0)
        	FTS_ERROR("write R8B fail, ret=%d", ret);
    }
    else
    {
    	ret = fts_i2c_write_reg(data->client, FTS_REG_CHARGER_MODE_EN, 0x00);
    	if (ret < 0)
        	FTS_ERROR("write R8B fail, ret=%d", ret);
    }
//sunjingtao@wind-mobi.com add at 20180801 end
...
//sunjingtao@wind-mobi modify for ffbm at 20180725 begin
    #ifdef BUILD_WIND_FOR_FACTORY
        if(!(NULL == strnstr(saved_command_line, "androidboot.mode=ffbm-01",strlen(saved_command_line))))
        {
		    printk("FTS bootmode is ffbm mode TP return\n");
		    return 0;
        }
	#endif
//sunjingtao@wind-mobi modify for ffbm at 20180525 end
...
//sunjingtao@wind-mobi modify for DVT1 at 20180517 begin
    if((NULL == strnstr(saved_command_line, "androidboot.mode=charger",strlen(saved_command_line))) || (NULL == strnstr(saved_command_line, "androidboot.mode=recovery",strlen(saved_command_line))))
    {
        ...
        printk("FTS bootmode is charge mode or recovery mode TP don't need to upgrade fw");
//sunjingtao@wind-mobi modify for DVT1 at 20180517 end
#endif
